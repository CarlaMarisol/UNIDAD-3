...
//Funcion para listar  tabla de productos en la página web
		...
		transaction.executeSql(sql,undefined, function(transaction,result){
			if(result.rows.length){
				...
				for(var i=0; i<result.rows.length; i++){
					...
					precio+' $</span></td><td><button type="button" id="A'+id+'" class="btn btn-success" onclick="editar()"><img src="Libreria/img/edit.png" /></button></td><td><button type="button" id="A'+id+'" class="btn btn-danger" onclick="eliminarRegistro()"><img src="Libreria/img/delete.png" /></button></td></tr>');
				}
			}else{
...
$("#borrarTodo").click(function(){
	if(!confirm("¿Quiere borrar la tabla?, los datos se perderán permanentemente",""))
		return;
	db.transaction(function(transaction){
		var sql="DROP TABLE productos";
		transaction.executeSql(sql,undefined,function(){
			alert("Tabla borrada, actualice la página")
		}, function(transaction, err){
			alert(err.message);
		})
	})
